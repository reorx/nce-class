...
新概念英语课堂教学辅助系统。当前进度：**M1 · 基础框架 + 老师端班级管理页面（读+写闭环，含登录鉴权）+ 课前配置页 + 课堂主界面（本地优先 + 结束课堂一次性提交入库，含默认分组回写/session 快照/事件流/背书作业/出勤）**。
需求/设计以 `kb/plans/2026-06-30-nce-class-m1-prd.md` 为准；班级管理写操作+登录见 `kb/plans/2026-07-01-nce-class-m1-management-wiring.md`；课堂运行时持久化见 `kb/plans/2026-07-02-nce-class-classroom-backend.md`。设计还原参考 `nce-class-v1-design/*.dc.html`（gitignored）。
...
server/   Express + TS · Drizzle ORM + SQLite (better-sqlite3)
  ...
  src/db/mutations.ts                  写操作（createClass/addStudent/deleteStudent 硬删/saveGrouping replace/commitSession 结束课堂单事务），事务封装
...
- **课前配置 + 课堂运行时持久化已完成**（本地优先 + 结束课堂一次性提交，见 `kb/plans/2026-07-02-nce-class-classroom-backend.md`）：开始课堂写本地 store（不发后端）；结束课堂在 `POST /api/classes/:id/sessions` 单事务里回写默认分组 §7.2 + 建 ClassSession/SessionGroup/SessionMembership 快照 + 批量 ScoreEvent/CheckRecord。还原 `课前配置.dc.html` / `课堂主界面.dc.html`，与 `tmp/goal-images/` 截图一致。
- 计时器归零后转「超时」正计（§7.3）已做；投屏实时多端同步 M1 不做（单端本地优先）。
